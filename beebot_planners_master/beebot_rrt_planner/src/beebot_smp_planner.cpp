#ifndef BEEBOT_SMP_PLANNER_CPP
#define BEEBOT_SMP_PLANNER_CPP

#include <beebot_rrt_planner/beebot_smp_planner.h>
...
/// ==================================================================================
geometry_msgs::PoseStamped beebotSMPPlanner::transformPose(geometry_msgs::PoseStamped init_pose){
...
/// ==================================================================================
void beebotSMPPlanner::setGoal(double x, double y, double theta, double toll, std::string goal_frame){
...
/// ==================================================================================
void beebotSMPPlanner::publishBlockedPlanner(bool status){
...
void beebotSMPPlanner::publishTree(){
...
/// ==================================================================================
void beebotSMPPlanner::publishSample(double x,double y, double theta, int ident){
...
/// ==================================================================================
/*
void beebotSMPPlanner::checkFrontLaserCollisionStatus(const CollisionStatus::ConstPtr& msg) {
...
/// ==================================================================================
/*
void beebotSMPPlanner::checkRearLaserCollisionStatus(const CollisionStatus::ConstPtr& msg){
...
/// ========================================================================================
/*
bool beebotSMPPlanner::SetDrivingDirection(spencer_nav_msgs::SetDrivingDirection::Request &req,spencer_nav_msgs::SetDrivingDirection::Response &res){
...
/// ==================================================================================
bool beebotSMPPlanner::setGlobalPathSupport( std::vector< geometry_msgs::PoseStamped > plan){

    support_bias_->clear_delete();
    support_->clear_delete();

    ROS_DEBUG("beebot_smp_planner setGlobalPathSupport");
    ...
    ROS_DEBUG("beebot_smp_planner setGlobalPathSupport dim path %d", (int)dim);
...
/// ==================================================================================
void beebotSMPPlanner::callbackObstacles(const nav_msgs::OccupancyGrid::ConstPtr& msg){
   ...
   ROS_DEBUG("Reading Obstacles positions..");
    ...
    obstacle_marker_.ns = "beebot_smp_planner";
...
/// ==================================================================================
/*
void beebotSMPPlanner::callbackAllTracks(const spencer_tracking_msgs::TrackedPersons::ConstPtr& msg){
...
/// publishNoPlan
/// ==================================================================================
void beebotSMPPlanner::publishNoPlan(int res){
...
/// publish Final plan
/// ==================================================================================
void beebotSMPPlanner::publishPlan(std::vector<geometry_msgs::PoseStamped>& plan){
...
/// ==================================================================================
void beebotSMPPlanner::publishPath(Trajectory *t){
...
/// To IMPLEMENT
    ...
    path_marker_.ns = "beebot_smp_planner";
    path_marker_.id = 1;
...
/// =================================================================================
int beebotSMPPlanner::saveTree(list <vertex_t *> *list_vertices){
...
/// wrap the angle
/// ==================================================================================
double beebotSMPPlanner::set_angle_to_range(double alpha, double min)
{
...
int beebotSMPPlanner::plan(Trajectory *traj,int type, geometry_msgs::PoseStamped& start){
...
bool beebotSMPPlanner::generatePlanOptimizeLocally(const geometry_msgs::PoseStamped& start,
...
/// generatePlan()
/// ==================================================================================
bool beebotSMPPlanner::generatePlan(const geometry_msgs::PoseStamped& start,
    const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan ){
...
/// ==================================================================================
bool beebotSMPPlanner::checkLayersAndPlan(const geometry_msgs::PoseStamped& start,
    const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan ){
...
/// makePlan()
/// ==================================================================================
bool beebotSMPPlanner::makePlan(const geometry_msgs::PoseStamped& start,
    const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan ){
...
/// ==================================================================================
void  beebotSMPPlanner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
    ...
    if(!initialized_){
        ...
        ros::NodeHandle node("~/beebot_rrt_planner");
        nh_ =  node;

        ROS_DEBUG("beebot_rrt_planner ŝtart initializing");
...
//        listener = costmap_ros_->getTransformListener();
      ...
      try{
        ...
        /// all the ROS DATA
        // setup publishers
        pub_planner_blocked_ = nh_.advertise<std_msgs::Bool>("/navigation/planner_blocked",5);
        ...
        // sub_obstacles_= nh_.subscribe("/move_base_node/global_costmap/costmap",1, &beebotSMPPlanner::callbackObstacles,this);

        // sub_all_agents_= nh_.subscribe("/spencer/perception/tracked_persons",1, &beebotSMPPlanner::callbackAllTracks,this);

        //frontLaserCollisionStatus_listener_ = nh_.subscribe("/control/collision/aggregated_front", 1, &beebotSMPPlanner::checkFrontLaserCollisionStatus, this);

        //rearLaserCollisionStatus_listener_  = nh_.subscribe("/control/collision/aggregated_rear", 1, &beebotSMPPlanner::checkRearLaserCollisionStatus, this);


        // subscriberss TODO!!!!
        sub_goal_=nh_.subscribe("/move_base_simple/goal",1,&beebotSMPPlanner::callbackSetGoal,this);


        sub_daryl_odom_= nh_.subscribe("/odom",5,&beebotSMPPlanner::callbackSetRobotPose,this);

//        srv_drivingdir = nh_.advertiseService("/set_driving_direction", &beebotSMPPlanner::SetDrivingDirection,this);
...
PLUGINLIB_EXPORT_CLASS(beebot_smp_planner::beebotSMPPlanner, nav_core::BaseGlobalPlanner);
